...
<body>
    <div class="container">
        ...
        <?php
        if ($this->action == "getSubscriber" && (!isset($this->data) || !$this->data || sizeof($this->data) == 0)) {
            echo "<p>The subscriber does not exists!</p>";
        } else if (isset($error)) {
            echo "<p>$error</p>";
        }
        ?>
        <form class="mb-5 needs-validation" action="index.php?controller=Subscriber&action=<?php echo isset($this->data['id']) ? "updateSubscriber" : "createSubscriber" ?>" method="post">
            ...
            <a id="return" class="btn btn-secondary" href="<?php echo "?controller=Subscriber&action=getAllSubscribers&action=getAllSubscribers"; ?>">Return</a>
        </form>
    </div>
</body>
